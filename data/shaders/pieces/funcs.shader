...
vec4 decodeRGBA8(uint icolor) {
	return vec4(float((icolor >>  0) & 0xffu), float((icolor >>  8) & 0xffu),
				float((icolor >> 16) & 0xffu), float((icolor >> 24) & 0xffu)) * (1.0 / 255.0);
}

vec3 decodeRGB8(uint icolor) {
	return vec3(float(icolor & 0xffu), float((icolor >> 8) & 0xffu), float((icolor >> 16) & 0xffu)) * (1.0 / 255.0);
}

uint encodeRGBA8(vec4 col) {
	return (uint(col.r * 255.0)) | (uint(col.g * 255.0) << 8) | (uint(col.b * 255.0) << 16) | ((uint(col.a * 255.0)) << 24);
}
...
uint encodeNormalHemiOct(vec3 n) {
	...
	return   uint((p.x + p.y) * 32767.0 + 32768.0) |
			(uint((p.x - p.y) * 16383.0 + 16384.0) << 16)
