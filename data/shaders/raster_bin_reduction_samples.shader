...
shared uint s_red_buffer[MAX_SAMPLES + 1];

// We only need 16-bit offsets?
shared int s_pixel_counts[BLOCK_COUNT * 64];
...
void computeCurrentPixelCounts(uint by) {
...
#else
		if((LIX & 15) >= 1)
			s_pixel_counts[LIX] += s_pixel_counts[LIX - 1];
		if((LIX & 15) >= 2)
			s_pixel_counts[LIX] += s_pixel_counts[LIX - 2];
		if((LIX & 15) >= 4)
			s_pixel_counts[LIX] += s_pixel_counts[LIX - 4];
		if((LIX & 15) >= 8)
			s_pixel_counts[LIX] += s_pixel_counts[LIX - 8];
		barrier();
	...
	if(LIX < BIN_SIZE * MAX_ROWS) {
		...
		value += (value << 16);
		// TODO: could be optimized (and generic subgroupInclusiveAdd should be used)
		temp = shuffleUpNV(value, 1, 32);
		if((LIX & 31) >= 1)
			value += temp & 0xffff0000;
		temp = shuffleUpNV(value, 2, 32);
		if((LIX & 31) >= 2)
			value += temp & 0xffff0000;
		temp = shuffleUpNV(value, 4, 32);
...
void generateBlocks(uint by) {
	...
	for(uint i = LIX & (LSIZE / 8 - 1); i < tri_count; i += LSIZE / 8) {
		...
		// subgrupInclusiveAdd is much slower than this... wtf?
		uint temp;
...
shared uint s_merged_count = 0;

// 0.06 ms on Sponza (simple)
// 0.3 ms on White Oak...
//
// On White Oak there are big differences in tri count between blocks
// maybe we should divide threads differently? but how? we would have to
// assign different block to each warp; Not obvious...
void mergeTriIndices(int bx, int by, int bx_step) {
	...
	if(LIX < 64)
		s_mini_buffer[LIX] = 0;
	barrier();
	for(uint istep = LSIZE / bx_step, i = (LIX & (istep - 1)); i < tri_count; i += istep) {
		uint tri_idx = g_scratch[soffset + i + MAX_BLOCK_TRIS].x & 0xffff;
		atomicOr(s_mini_buffer[tri_idx >> 5], 1 << (tri_idx & 31));
	}
	barrier();
	if(LIX < 64) {
		uint cur_count = bitCount(s_mini_buffer[LIX]), temp;
		temp = shuffleUpNV(cur_count, 1, 32);
		if((LIX & 31) >= 1)
			cur_count += temp;
		temp = shuffleUpNV(cur_count, 2, 32);
		if((LIX & 31) >= 2)
			cur_count += temp;
		temp = shuffleUpNV(cur_count, 4, 32);
		if((LIX & 31) >= 4)
			cur_count += temp;
		temp = shuffleUpNV(cur_count, 8, 32);
		if((LIX & 31) >= 8)
			cur_count += temp;
		temp = shuffleUpNV(cur_count, 16, 32);
		if((LIX & 31) >= 16)
			cur_count += temp;
		s_mini_buffer[LIX + 64] = cur_count;
	}
	barrier();
	if(LIX < 32) {
		uint value = s_mini_buffer[LIX + 64 + 32] + s_mini_buffer[64 + 31];
		s_mini_buffer[LIX + 64 + 32] = value;
		if(LIX == 31)
			s_merged_count = value;
	}
	// Now we can easily compute new offset for each triangle:
	// s_mini_buffer[LIX + 64] + bitCount(masked_trimask)
}
...
// thread count: bx_step * BIN_SIZE
void loadReductionSamples(int bx, int by, int bx_step) {
	bx += int(LIX >> 6);
	...
	uint tri_count = BLOCK_TRI_COUNT(bx);
	uint block_offset = BLOCK_FRAG_OFFSET(bx) & 0xffff;
	uint pixel_offset = block_offset + (s_pixel_counts[y * 64 + x] >> 16);
	...
	uint prev_ids[3] = {~0u, ~0u, ~0u};

	// TODO: we're loading block-tri data twice
	for(uint i = 0; i < tri_count; i += 32) {
		...
		while(j != -1) {
			...
			if(prev_ids[2] != ~0u)
				s_red_buffer[pixel_offset++] = prev_ids[2];
			prev_ids[2] = prev_ids[1];
			prev_ids[1] = prev_ids[0];
			prev_ids[0] = tri_offset;
		}
	}
	for(int i = 2; i >= 0; i--)
		if(prev_ids[i] != ~0u)
			s_red_buffer[pixel_offset++] = prev_ids[i];

	// TODO: handle it properly: full sort?
	if(pixel_bit == 0)
		s_red_buffer[block_offset + (s_pixel_counts[y * 64 + x] >> 16)] = ~0u;
}

// TODO: optimize; it should be possible to use all threads here
void reduceSamples(int bx, int by, int bx_step) {
	...
	uint tri_count = BLOCK_TRI_COUNT(bx);
	uint block_offset = BLOCK_FRAG_OFFSET(bx) & 0xffff;
	uint pixel_count = s_pixel_counts[y * 64 + x];
	uint pixel_offset = block_offset + (pixel_count >> 16);
	pixel_count &= 0xffff;

	vec3 out_color = vec3(0);
	float out_transparency = 1.0;

	if(pixel_count > 0 && s_red_buffer[pixel_offset] == ~0u) {
	...
	for(uint i = 0; i < pixel_count; i++) {
		uint sample_id = s_red_buffer[pixel_offset + i];
		vec4 cur_color = decodeRGBA8(s_buffer[sample_id]);
...
// Note: this allows simple load balancing during sampling
// it will be useful when performing heterogenous computations
shared uint s_shading_counter = 0;
...
void shadeSamples(uint bx, uint by) {
	...
	uint warp_id = LIX & 31;
	uint i, wi = warp_id == 0 ? atomicAdd(s_shading_counter, 32) : 0;
	i = shuffleNV(wi, 0, 32) + warp_id;

	// TODO: problem:
	// when we're performing sampling in the following way, then each thread may process uneven # of samples...
	// how are we supposed to store them in registers ?
	...
	while(i < sample_count) {
		...
		wi = warp_id == 0 ? atomicAdd(s_shading_counter, 32) : 0;
		i = shuffleNV(wi, 0, 32) + warp_id;
	}
}
...
void rasterFragmentCounts(int by) {
	for(uint i = LIX; i < BIN_SIZE * MAX_ROWS; i += LSIZE) {
		...
		//count0 = s_pixel_counts[(pixel_pos.y & 7) * 64 + pixel_pos.x] >> 16;
		//count1 = count0;

		vec3 color = vec3(count0, count1, 0) / float(MAX_SAMPLES);
		if(count0 == 0xffff)
...
void rasterBin(int bin_id) {
	...
	for(int by = 0; by < BLOCK_COUNT; by++) {
		...
		for(int bx = 0; bx < BLOCK_COUNT; bx += bx_step) {
			barrier();
			/*mergeTriIndices(bx, by, bx_step);
			barrier();
			if(s_merged_count > 256) {
				rasterInvalidBlockRow(by, vec3(0.2, 0.2, 0.0));
				continue;
			}*/
			...
			if(LIX < bx_step * BIN_SIZE)
				loadReductionSamples(bx, by, bx_step);
			shadeSamples(bx, by);
			barrier();
			UPDATE_CLOCK(3);
